...
var data1 = datas.filter((e) => {
  ...
  {data1.map((i) => {
    ...
    <Grid item xs={12} sm={12} md={4} key={i._id}>
      ...
      <Card  style={localStorage.getItem("darkmode") === "dark"? {background:"#525252" ,color:"white",marginTop:"20px"}: {background:"#F3F3F3",marginTop:"20px"}} sx={{ maxWidth: 345 }}>
      <CardMedia
        component="img"
        height="200"
        image={i.image}
      ...
      <CardActions>
        <Button size="small" style={{background:"#0E1F4B",color:"white"}}><Link style={{textDecoration:"none",color:"white"}} to={`/blog/${i._id}`}>See more</Link></Button>
        { a.length === 0 ?
