...
const PersonalPage = () => {
    ...
    const onHandleBlog = (e) => {
      e.preventDefault();
      if(formToggle1 === true){
      if(data4.star_name === "" || data4.description === ""||data4.image === ""){
  ...
  return (
    <div  style ={localStorage.getItem("darkmode") ==="dark" ? {minHeight:"100vh",background:"#222121",color:"white"}:null}>
      ...
      <Container maxWidth="xl">
        <Toolbar disableGutters>
            ...
            >
              {pages.map((page) => (
                <MenuItem key={page} onClick={handleCloseNavMenu}>
                  <Typography textAlign="center"><Link to={page.link} style={{textDecoration:"none",color:"black"}}>{page.name}</Link></Typography>
                </MenuItem>
              ))}
            </Menu>
          </Box>
         ...
         {!localStorage.getItem("bloguser id")?null: <Box sx={{ flexGrow: 0 }}>
            <Tooltip title="Open settings">
              <IconButton onClick={handleOpenUserMenu} style={{background:"white"}} sx={{ p: 0 }}>
                <Avatar alt="Remy Sharp" src="https://img.icons8.com/ios/50/000000/user--v1.png" />
              </IconButton>
            </Tooltip>
            <Menu
              sx={{ mt: '45px' }}
            ...
            >
                 <MenuItem disabled="true"  >
                  <Typography textAlign="center">{localStorage.getItem("bloguser email")}</Typography>
                </MenuItem>
    ...
    <Grid container>
       ...
       <Typography variant="h3">Blog</Typography>
           ...
           <Button style={{background:"#4189B9", color:"white", marginTop:"20px", marginLeft:"10px"}} onClick = {() => showForm()}>{formToggle2 === false ? "Show" : "Hide"} Form</Button>
           <Button style={{background:"#4189B9", color:"white", marginTop:"20px", marginLeft:"10px"}} onClick = {() => showBlog()}>{formToggle3 === false ? "Show" : "Hide"} Blog</Button>
           <Grid xs={8}>
           ...
           <TextField
            ...
            InputLabelProps={localStorage.getItem("darkmode") ==="dark" ?{ className: classes1.textField1 }:null}
                ...
                onChange={(e)=> {setData4({...data4,star_name:e.target.value})}}
                autoFocus
              ...
              <TextField
               ...
               InputLabelProps={localStorage.getItem("darkmode") ==="dark" ?{ className: classes1.textField1 }:null}
                ...
                onChange={(e)=> {setData4({...data4,description:e.target.value})}}
              />
             ...
             <TextField
              ...
              InputLabelProps={localStorage.getItem("darkmode") ==="dark" ?{ className: classes1.textField1 }:null}
                ...
                // name="Image-Link"
                value={data4.image}
                onChange={(e)=> {setData4({...data4,image:e.target.value})}}
              />
              ...
              <Button
                ...
                variant="contained"
                sx={{ mt: 3, mb: 2 }}
              >{ formToggle1 === true ? 'Add New Blog' : 'Update Existing Blog' 
              }
              </Button> 
            </form>
            }
            ...
            { formToggle3 === false ? null : (blog_data  ? blog_data.map((i) => {
              return(
                <Card style={localStorage.getItem("darkmode") === "dark"? {background:"#525252" ,color:"white",marginTop:"20px"}: {background:"#F3F3F3",marginTop:"20px"}}>
      <CardMedia
        component="img"
        alt="green iguana"
      ...
      <CardActions>
        <Button style={{background:"#0E1F4B",color:"white"}} onClick={() => onHandleBlogUpdate(i._id)}>Update Blog</Button>
        <Button style={{background:"#0E1F4B",color:"white"}} onClick={() => onHandleBlogDelete(i._id)}>Delete Blog</Button>
      </CardActions>
    </Card>
              )
            }):<p>No Blog Created</p>)}
          ...
          <Grid xs={4} >
             <div align="center">
               ...
               <hr/>
              <p style={{marginTop:"20px"}}>{localStorage.getItem("bloguser email")}</p>
              <Button style={{background:"#403F71",color:"white"}} onClick={(e) =>  onUserUpdate(e)}>{formToggle === false ?"Update Profile": "Cancel" }</Button>
              &nbsp;&nbsp;<Button
              ...
              {formToggle === true ? 
              <form style={{marginLeft:"20px"}} onSubmit={(e) => onHandleUserUpdate (e)} >
            <p>{error}</p>
            <label>Email</label>   
           <TextField
            InputProps={localStorage.getItem("darkmode") ==="dark" ?{style:{borderBottom:"1px solid white"},className:classes1.input }:null}
